...
const Navbar = ({ setSignin,setCurrentPage,currentPage,setIsMenuOpen,isMenuOpen}) => {
 ...
 <div className=' sticky z-20 bg-black text-white  text-[14px] top-0'>
     <div className='flex sticky z-20 bg-black text-white  items-center justify-between text-[14px] top-0 h-[10vh] left-0 px-6 md:px-8 lg:px-30  w-full'>
      ...
      <div className='hidden md:flex gap-2'>
        <span className={`cursor-pointer ${currentPage==='home'?'text-[#C8F400] cursor-pointer':'' }`} onClick={()=>{
          setCurrentPage('home')
          localStorage.setItem('Current-Page',JSON.stringify('home'));
          }}>Home</span>
        <span className={`cursor-pointer ${currentPage==='shop'?'text-[#C8F400] cursor-pointer':'' }`} onClick={()=>{
          setCurrentPage('shop')
          localStorage.setItem('Current-Page',JSON.stringify('shop'));
          }}>Shop</span>
        <span className={`cursor-pointer ${currentPage==='about'?'text-[#C8F400] cursor-pointer':'' }`} onClick={()=>{
      ...
      <div className='flex gap-3'>
        ...
        <span 
          className='border-[.5px] border-white/30 px-2.5 py-2 flex items-center rounded-md'
          onClick={()=>{
